"""Sketch-to-photo conversion helper for VeriFace.

This module is sourced from the extracted Face-Sketch-SCG package and
reused by the FastAPI sketch search endpoint.
"""
...
try:
    ...
    sys.exit(
        ...
        f"Install requirements first:  pip install -r requirements.txt --break-system-packages"
    )

from sketch_models.sketch_model.networks import ResnetGenerator, apply_norm
...
def save_output_photo(output_tensor: "torch.Tensor", output_path: str, original_size: tuple[int, int]) -> None:
    ...
    array = array.transpose(1, 2, 0).astype("uint8")
